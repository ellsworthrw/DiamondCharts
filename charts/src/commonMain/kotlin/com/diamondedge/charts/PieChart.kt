...
*/
    ...
    override fun draw(g: GraphicsContext) {
        ...
        if (dsCount > 8)
            ...
            rows = 2
        val cols = ceil(dsCount.toDouble() / rows).toInt()
        ...
        for (series in 0 until dsCount) {
            ...
            log.d { "x=$x y=$y right=${x + w} bottom=${y + w}" }
            for (i in 0 until dataCount) {
                value = data.getValue(series, i) / total
                if (i == dataCount - 1)
                    angle = 360 - startAngle   // last angle: make sure it finishes circle
                else
...
//                        xPts[j] = xc + (r * cos(theta)).toInt()
//                        yPts[j] = yc - (r * sin(theta)).toInt()
//                        theta += angleIncr
//                    }
