...
//TODO:        symbol++
    ...
    private fun recalcNumPoints() {
        ...
        var dataCount = ddata.dataCount
        var dataPt = 0
        dataCount = ceil(1.0 * dataCount / period).toInt()
    ...
    private fun recalcRange() {
        ...
        dataCount = ceil((maxXdata - minXdata) / valueRange).toInt()
        ...
        while (i < dataCount && maxX < maxXdata) {
            minX = maxX
            maxX += nextInc(minX, this.periodStyle and 0xFF)
            if (maxX > maxXdata)
      ...
      if( i == 0 && numPts > 0 )  // first group inclusive of minX and maxX points
        ...
        */
            if (numPts > 0) {
                ...
                values.copyInto(value, 0, i, i + numPts)
                /*
    ...
    private fun calculate(value: DoubleArray): Double {
        when (functionType) {
            ...
            VAR -> return Statistics.variance(value)
    ...
    companion object {
        ...
        val SECOND = RANGE or DateUtil.SECOND_FIELD
        val MINUTE = RANGE or DateUtil.MINUTE_FIELD
        val HOUR = RANGE or DateUtil.HOUR_FIELD
        val DAY = RANGE or DateUtil.DAY_FIELD
        val WEEK = RANGE or DateUtil.WEEK_FIELD
        val MONTH = RANGE or DateUtil.MONTH_FIELD
        val YEAR = RANGE or DateUtil.YEAR_FIELD

        private var symbol = SymbolType.SQUARE_SMALL

        /**
         * Distance from [pos] (in days) to the next boundary for the given calendar [field]
         * (the low byte of a period-style constant). Calendar-aware for WEEK/MONTH/YEAR.
         */
        internal fun nextInc(pos: Double, field: Int): Double {
            return DateUtil.periodIncrement(pos, field)
        }
