...
/**
 * Grows or shrinks this list to [newSize], padding new slots with `null`. Multiplatform replacement
 * for `java.util.Vector.setSize`, which the data tables were originally built on.
 */
internal fun <T> MutableList<T?>.setSize(newSize: Int) {
...
open class DefaultData(override val id: Any = "", val seriesType: Int) : ChartData {
    protected var data = ArrayList<ArrayList<Any?>?>()

    override var seriesCount: Int
        get() = (columnCount - 1) / valueCount
        set(value) {
            if (value == 0)
                data[0] = ArrayList<Any?>()
    ...
    private val graphicAttributesList = ArrayList<GraphicAttributes?>()
