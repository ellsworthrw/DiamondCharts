...
import com.diamondedge.chart.Font
import com.diamondedge.chart.FontFace
import com.diamondedge.chart.FontMetrics
import com.diamondedge.chart.FontStyle
import com.diamondedge.chart.Gradient
import com.diamondedge.chart.GradientType
import com.diamondedge.chart.GraphicsContext
import com.diamondedge.chart.StrokeStyle
...
class ComposeGC(private val g: Canvas, private val density: Density) : GraphicsContext {
    ...
    override var font: Font = Font.Default
        set(value) {
            field = value
            textSize = value.size
            _paint.asFrameworkPaint().typeface = installTypeface(value)
    ...
    private var textSize: Float = 0f
        set(value) {
            field = value
            _paint.asFrameworkPaint().textSize = dpToPx(value)
        }

    init {
        val a = density.density
        _paint.asFrameworkPaint().typeface = installTypeface(font)
    ...
    private fun calcFontmetrics(): FontMetrics {
        val fm = _paint.asFrameworkPaint().fontMetricsInt
    ...
    private fun installFontMetrics(font: Font): FontMetrics {
        return font.fontMetrics ?: run {
            ...
            val origTypeface = _paint.asFrameworkPaint().typeface
            val typeface = installTypeface(font)
            _paint.asFrameworkPaint().textSize = dpToPx(font.size)
            _paint.asFrameworkPaint().typeface = typeface
            val fm = calcFontmetrics()
            font.fontMetrics = fm
            _paint.asFrameworkPaint().textSize = dpToPx(origTextSize)
            _paint.asFrameworkPaint().typeface = origTypeface
            fm
        }
    }
    ...
    override var stroke: Any
        get() = Stroke(_paint.strokeWidth, pathEffect = _paint.pathEffect)
        set(value) {
            if (value is Stroke) {
                _paint.strokeWidth = value.width
                _paint.pathEffect = value.pathEffect
            }
        }
    ...
    override fun applyGradient(gradient: Gradient, alpha: Float) {
        ...
        val brush = when (gradient.style) {
            GradientType.TopToBottom -> Brush.verticalGradient(
                *colors.toTypedArray(),
                startY = gradient.bounds.y.toFloat(),
                endY = gradient.bounds.bottom.toFloat()
            )
            GradientType.Radial -> {
                ...
                Brush.radialGradient(
                    *colors.toTypedArray(),
                    ...
                    radius = maxOf(gradient.bounds.width, gradient.bounds.height).toFloat()
                )
            }
            GradientType.LeftToRight -> Brush.horizontalGradient(
                *colors.toTypedArray(),
                startX = gradient.bounds.x.toFloat(),
                endX = gradient.bounds.right.toFloat()
            )
        }
        brush.applyTo(Size(gradient.bounds.width.toFloat(), gradient.bounds.height.toFloat()), _paint, alpha)
    }
    ...
    override fun stringWidth(str: String): Int {
        return _paint.asFrameworkPaint().measureText(str).toInt()
    }
    ...
    fun drawLine(x1: Float, y1: Float, x2: Float, y2: Float) {
        g.drawLine(Offset(x1, y1), Offset(x2, y2), _paint)
    }
    ...
    fun fillRect(x: Float, y: Float, width: Float, height: Float) {
        ...
        g.drawRect(x, y, x + width, y + height, _paint)
    }
    ...
    private fun drawRoundedRect(x: Float, y: Float, width: Float, height: Float, radiusX: Float, radiusY: Float) {
        g.drawRoundRect(x, y, x + width, y + height, radiusX, radiusY, _paint)
    }
    ...
    fun drawOval(x: Float, y: Float, width: Float, height: Float) {
        _paint.style = PaintingStyle.Stroke
        g.drawOval(x, y, x + width, y + height, _paint)
    }
    ...
    fun drawArc(x: Float, y: Float, width: Float, height: Float, startAngle: Float, arcAngle: Float) {
        _paint.style = PaintingStyle.Stroke
        g.drawArc(x, y, x + width, y + height, startAngle, arcAngle, true, _paint)
    }
    ...
    fun drawString(str: String, x: Float, y: Float) {
        g.nativeCanvas.drawText(str, x, y, _paint.asFrameworkPaint())
    }
