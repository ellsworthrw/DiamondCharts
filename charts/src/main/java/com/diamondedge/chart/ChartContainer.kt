...
abstract class ChartContainer {
    ...
    fun draw(g: GraphicsContext, afterCalculations: ((GraphicsContext) -> Unit)? = null) {
        ...
        var chartWidth = d.width - leftMargin - rightMargin - vertAxisWidth - vertAxis2Width
        var chartHeight = d.height - topMargin - bottomMargin - horAxisHeight - chartTitleHeight
        log.d { "chartWidth: $chartWidth contWidth: ${d.width} leftMargin: $leftMargin rightMargin: $rightMargin vertAxisWidth: $vertAxisWidth vertAxis2Width: $vertAxis2Width" }
        if (legendPosition != LEGEND_NONE) {
            ...
            if (legendPosition == LEGEND_RIGHT)
                chartWidth -= legendSize!!.width + 5
            else if (legendPosition == LEGEND_BOTTOM) {
                legend!!.orientation = Legend.HORIZONTAL
                chartHeight -= legendSize!!.height + 5
            }
        }
        val chartLeft = leftMargin + vertAxisWidth
        val chartBottom = topMargin + chartTitleHeight + chartHeight
        // draw axis first since it sets up scaling for charts
        vertAxis?.apply {
            setBounds(chartLeft, chartBottom, vertAxisWidth, chartHeight)
            calcMetrics(height, g, majorTickFont)
        }
        horizontalAxis?.apply {
            setBounds(chartLeft, chartBottom, chartWidth, getThickness(g))
        ...
        legend?.let { legend ->
            ...
            if (legendPosition == LEGEND_RIGHT) {
                var y = chartBottom - chartHeight + (chartHeight - legendSize!!.height) / 2
                if (legendSize.height > chartHeight)
                    y = chartBottom - chartHeight
                legend.setLocation(chartLeft + chartWidth + 5, y)
            } else if (legendPosition == LEGEND_BOTTOM)
                legend.setLocation(chartLeft + (chartWidth - legendSize!!.width) / 2, chartBottom + horAxisHeight + 5)
        }
        ...
        gridLines.draw(g, chartLeft, chartBottom, chartWidth, chartHeight)

        for (obj in charts) {
            if (obj is Chart) {
                if (vertAxis == null && horizontalAxis == null)
                    obj.setBounds(chartLeft, chartBottom - chartHeight, chartWidth, chartHeight)
            }
            obj.draw(g)
        }
